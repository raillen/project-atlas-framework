...
var commandRegistry = map[string]CommandInfo{
	...
	"agent": {
		Name: "agent",
		Category: "Harness",
		Summary: "Headless Prumo-native agent harness (run/resume/handoff)",
		Usage: "prumo agent <run|resume|handoff> [flags]",
		Description: "Runs the NativeAgent state machine headlessly: context, model, tools, permissions, checkpoints. Provider-neutral (fake|openai-compat); external agents via AgentProvider adapters.",
		Flags: []string{
			"--goal <text>        Goal for agent run (default: headless run)",
			"--path <dir>         Workspace root (default: .)",
			"--provider <name>    fake|openai-compat (default: fake)",
			"--model <id>         Model id for real providers",
			"--base-url <url>     Base URL for openai-compat",
