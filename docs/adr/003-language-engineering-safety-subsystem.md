...
- **Escape-Hatch Registry**: Banned constructs (e.g. `reinterpret_cast`, raw `malloc`/`free`, `(void*)`, `@ptrCast`, `@trusted`, suppressions) cannot be introduced without formal registration in `.atlas/escape-hatches.json` or inline annotations with explicit safety invariants and quarantine boundaries (`schemas/escape-hatch.schema.json`).
- **Native Go Tooling**: Embedded `atlas tool check-escape-hatches` scanner provides instant verification in CLI, pre-commit hooks, and CI pipelines.
...
- `src/project_atlas/resources/catalog/skills.json`
