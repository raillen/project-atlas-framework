...
def _selector_matches(item: dict[str, Any], profile: ProjectProfile) -> tuple[bool, list[str]]:
    ...
    return matched, reasons if matched else []
...
def resolve(profile: ProjectProfile) -> Resolution:
    ...
    for skill_id, item in skills_catalog.items():
        matched, why = _selector_matches(item, profile)
        if matched:
